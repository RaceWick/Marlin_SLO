  ...
  void menu_advanced_filament() {
    ...
    BACK_ITEM(MSG_ADVANCED_SETTINGS);
      ...
      if (parser.volumetric_enabled) {
        EDIT_ITEM_FAST(float42_52, MSG_FILAMENT_DIAM, &planner.filament_size[active_extruder], 1.5f, 20.00f, planner.calculate_volumetric_multipliers);
        #if HAS_MULTI_EXTRUDER
          EXTRUDER_LOOP()
            EDIT_ITEM_FAST_N(float42_52, e, MSG_FILAMENT_DIAM_E, &planner.filament_size[e], 1.5f, 20.00f, planner.calculate_volumetric_multipliers);
        #endif
      }
    #endif
